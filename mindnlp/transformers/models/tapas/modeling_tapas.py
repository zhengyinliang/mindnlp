...
# import torch
# import ops.utils.checkpoint
# from torch import nn
# from ops.nn import BCEWithLogitsLoss, CrossEntropyLoss, MSELoss
import numpy as np
import mindspore
...
#     logger.warning(
#         f"You are using torch=={ops.__version__}, but torch>=1.12.0 is required to use "
#         "TapasModel. Please upgrade ops."
#     )
...
def load_tf_weights_in_tapas(model, config, tf_checkpoint_path):
    ...
    for name, array in zip(names, arrays):
        ...
        pointer.data = ops.from_numpy(array)
    return model
...
class TapasForQuestionAnswering(TapasPreTrainedModel):
    def __init__(self, config: TapasConfig):
        ...
        if config.init_cell_selection_weights_to_zero:
            ...
            self.output_weights = mindspore.Parameter(ops.zeros(config.hidden_size),'output_weights')
            self.column_output_weights = mindspore.Parameter(ops.zeros(config.hidden_size),'column_output_weights')
        else:
            self.output_weights = mindspore.Parameter(ops.empty(config.hidden_size),'output_weights')
            nn.init.normal_(
                self.output_weights, std=config.initializer_range
            )  # here, a truncated normal is used in the original implementation
            self.column_output_weights = mindspore.Parameter(ops.empty(config.hidden_size),'column_output_weights')
            nn.init.normal_(
                self.column_output_weights, std=config.initializer_range
            )  # here, a truncated normal is used in the original implementation
        self.output_bias = mindspore.Parameter(ops.zeros([]),'output_bias')
        self.column_output_bias = mindspore.Parameter(ops.zeros([]),'column_output_bias')
    ...
    def construct(
        ...
        row_index = IndexMap(
            indices=ops.min(row_ids, ops.as_tensor(self.config.max_num_rows - 1)),
            num_segments=self.config.max_num_rows,
            batch_dims=1,
        )
        col_index = IndexMap(
            indices=ops.min(column_ids, ops.as_tensor(self.config.max_num_columns - 1)),
        ...
        if labels is not None:
            ...
            dist_per_token = ops.distributions.Bernoulli(logits=logits)
            ...
            if not self.config.select_one_column:
                weight = ops.where(
                    labels == 0,
                    ops.ones_like(labels, dtype=ops.float32),
                    self.config.positive_label_weight * ops.ones_like(labels, dtype=ops.float32),
                )
                selection_loss_per_token = -dist_per_token.log_prob(labels) * weight
                selection_loss_per_example = ops.sum(selection_loss_per_token * input_mask_float, axis=1) / (
                    ops.sum(input_mask_float, axis=1) + EPSILON_ZERO_DIVISION
                )
            else:
    ...
    def construct(
        ...
        if labels is not None:
            ...
            if self.config.problem_type == "regression":
                ...
                loss = ops.binary.cross_entropy_with_logits(logits, labels)
        if not return_dict:
...
def flatten(index, name="segmented_flatten"):
    ...
    *num_segments* * (k - 1). The result is a tensor with *num_segments* multiplied by the number of elements in the
    ...
    batch_size = ops.prod(mindspore.Tensor(list(index.batch_shape())))
...
def range_index_map(batch_shape, num_segments, name="range_index_map"):
    ...
    new_tensor = ops.cat(
        [ops.ones_like(batch_shape, dtype=mindspore.int64), num_segments.unsqueeze(axis=0)],
        axis=0,
    )
    ...
    multiples = ops.cat([batch_shape, ops.as_tensor([1])], axis=0)
...
def _segment_reduce(values, index, segment_reduce_fn, name):
    ...
    flattened_shape = ops.cat(
        [ops.as_tensor([-1], dtype=mindspore.int64), ops.as_tensor(vector_shape, dtype=mindspore.int64)], axis=0
    )
    # changed "view" by "reshape" in the following line
    flat_values = values.reshape(flattened_shape.tolist())

    out = ops.zeros(int(flat_index.num_segments), dtype=ops.float)
    segment_means = out.scatter_reduce(
        axis=0, index=flat_index.indices.long(), src=flat_values.float(), reduce=segment_reduce_fn, include_self=False
    )
    ...
    new_shape = ops.cat(
        [
            ...
            ops.as_tensor([index.num_segments], dtype=mindspore.int64),
...
def compute_column_logits(
    ...
    column_logits += CLOSE_ENOUGH_TO_LOG_ZERO * ops.as_tensor(
        is_padding, dtype=ops.float32
    )

    if not allow_empty_column_selection:
        column_logits += CLOSE_ENOUGH_TO_LOG_ZERO * ops.as_tensor(
            ops.eq(out_index.indices, 0), dtype=ops.float32
        )
...
def _single_column_cell_selection_loss(token_logits, column_logits, labels, cell_index, col_index, cell_mask):
    ...
    labels_per_column, _ = reduce_sum(ops.as_tensor(labels, dtype=ops.float32), col_index)
    ...
    column_label = ops.argmax(labels_per_column, axis=-1)  # shape (batch_size,)
    ...
    column_dist = ops.distributions.Categorical(logits=column_logits)  # shape (batch_size, max_num_cols)
    ...
    column_mask = ops.as_tensor(
        ops.eq(column_id_for_cells, ops.unsqueeze(column_label, axis=-1)),
        dtype=ops.float32,
    )

    # Compute the log-likelihood for cells, but only for the selected column.
    cell_dist = ops.distributions.Bernoulli(logits=logits_per_cell)  # shape (batch_size, 64*32)
    cell_log_prob = cell_dist.log_prob(labels_per_cell.type(ops.float32))  # shape(batch_size, 64*32)

    cell_loss = -ops.sum(cell_log_prob * column_mask * cell_mask, axis=1)

    # We need to normalize the loss by the number of cells in the column.
    cell_loss /= ops.sum(column_mask * cell_mask, axis=1) + EPSILON_ZERO_DIVISION
    ...
    selected_column_id = ops.as_tensor(
        ops.argmax(column_logits, axis=-1), dtype=mindspore.int64
    )  # shape (batch_size,)
    ...
    selected_column_mask = ops.as_tensor(
        ops.eq(column_id_for_cells, ops.unsqueeze(selected_column_id, axis=-1)),
        dtype=ops.float32,
    )
...
def _calculate_aggregate_mask(answer, pooled_output, cell_selection_preference, labels, aggregation_classifier):
    ...
    dist_aggregation = ops.distributions.categorical.Categorical(logits=logits_aggregation)
    # Index 0 corresponds to "no aggregation".
    aggregation_ops_total_mass = ops.sum(dist_aggregation.probs[:, 1:], axis=1)
    ...
    is_cell_supervision_available = ops.sum(labels, axis=1) > 0
    ...
    aggregate_mask = ops.where(
        ...
        ops.zeros_like(aggregate_mask_init, dtype=ops.float32),
        aggregate_mask_init,
    )
...
def _calculate_aggregation_loss_known(
    ...
    one_hot_labels = ops.one_hot(target_aggregation, num_classes=num_aggregation_labels).type(ops.float32)
    log_probs = ops.log_softmax(logits_aggregation, axis=-1)

    # mindspore.Tensor[batch_size]
    per_example_aggregation_intermediate = -ops.sum(one_hot_labels * log_probs, axis=-1)
    if use_answer_as_supervision:
...
def _calculate_expected_result(
    ...
    if config.use_gumbel_for_cells:
        gumbel_dist = ops.distributions.RelaxedBernoulli(
    ...
    count_result = ops.sum(scaled_probability_per_cell, axis=1)
    ...
    sum_result = ops.sum(scaled_probability_per_cell * numeric_values_masked, axis=1)
    ...
    if avg_approximation == AverageApproximationFunction.RATIO:
        ...
        ex = ops.sum(scaled_probability_per_cell, axis=1, keepdim=True) - scaled_probability_per_cell + 1
        average_result = ops.sum(numeric_values_masked * scaled_probability_per_cell / ex, axis=1)
    elif avg_approximation == AverageApproximationFunction.SECOND_ORDER:
        # The sum of all probabilities except that correspond to other cells
        ex = ops.sum(scaled_probability_per_cell, axis=1, keepdim=True) - scaled_probability_per_cell + 1
        pointwise_var = scaled_probability_per_cell * (1 - scaled_probability_per_cell)
        var = ops.sum(pointwise_var, axis=1, keepdim=True) - pointwise_var

        multiplier = (var / ops.square(ex) + 1) / ex
        average_result = ops.sum(numeric_values_masked * scaled_probability_per_cell * multiplier, axis=1)
    else:
    ...
    if config.use_gumbel_for_aggregation:
        gumbel_dist = ops.distributions.RelaxedOneHotCategorical(
    ...
    all_results = ops.cat(
        [
            ops.unsqueeze(sum_result, axis=1),
            ops.unsqueeze(average_result, axis=1),
            ops.unsqueeze(count_result, axis=1),
        ],
        axis=1,
    )

    expected_result = ops.sum(all_results * aggregation_op_only_probs, axis=1)
...
def _calculate_regression_loss(
    ...
    if config.answer_loss_cutoff is None:
        large_answer_loss_mask = ops.ones_like(per_example_answer_loss, dtype=ops.float32)

    else:
        large_answer_loss_mask = ops.where(
            per_example_answer_loss > config.answer_loss_cutoff,
            ops.zeros_like(per_example_answer_loss, dtype=ops.float32),
            ops.ones_like(per_example_answer_loss, dtype=ops.float32),
        )
