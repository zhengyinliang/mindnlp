...
class TapasModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        ...
        expected_slice = ops.tensor([[0.987518311, -0.970520139, -0.994303405]])
    ...
    def test_training_question_answering_head_weak_supervision(self):
        ...
        float_answer = ops.FloatTensor(float_answer)
        ...
        expected_shape = ops.Size((2, 4))
        self.assertEqual(logits_aggregation.shape, expected_shape)
        expected_slice = ops.tensor([-4.0538, 40.0304, -5.3554, 23.3965])

        self.assertTrue(np.allclose(logits_aggregation[1, -4:].asnumpy(), expected_slice.asnumpy(), atol=1e-4))
...
class TapasUtilitiesTest(unittest.TestCase):
    ...
    def test_flatten(self):
        ...
        shape = [3, 4, 5]
        batched_index = IndexMap(indices=ops.zeros(shape).type(ops.LongTensor), num_segments=1, batch_dims=3)
